...
### 1. Prepare UniProt reference files
`run/0-download_and_parse_source.slurm`
* Download up-to-date UniProt source file (`download_uniprot.sh`)
* Run `1-parse_source_data.py` to process reference files downloaded from UniProt FTP site
    ```
...
**2.1 Extract meta information from PDB:**  
* Extract meta information from PDB: `run/prep_pdb_info.slurm`
    * Raw PDB data: `/share/yu/resources/local_resources/pdb/data/data/pdb`
    * PDB-bundle data: `/share/yu/resources/local_resources/pdb/data/data/pdb_like` (large PDB structures saved as [pdb-bundle TAR format](https://www.rcsb.org/docs/general-help/structures-without-legacy-pdb-format-files))

**2.2 `run/1-prepare_raw_interactome.sh` performs the following steps:**
* Download PPI source data (`download_ppi_source.sh`)
Collect datasets from sources of interest. Active datasets will be downloaded from the source and inactive dataset will be copied from our self-maintained cache directory. Raw data files will be saved at `$UPDATE_DIR/data/parseTargets`

The following data sources are included (**Last update: 2026.5.18**)
* Active:
  ...
  * PDB: copy up-to-date IRES files generated by in-house `nightly_script`
    * `/share/yu/resources/local_resources/pdb/parsed_file/ires_perpdb_w_sifts_mapping.txt`
* Inactive:
  * DIP (`dip20170205.txt`)
  ...
  * MIPS (`mppi` xml format)
  ...
  ```
* Run `2-prepare_dataset.py` to prepare protein interaction data from sources of interest. The following steps will be processed by running the script:
...
`run/2-run_id_mapping.sh`: 
Run `3-create_idmapping.py` to parse `idmapping.dat.gz` from UniProt FTP site and generate ID mapping dictionary from source IDs to UniProt IDs
...
> **Change logs**  
**2026-05**:  
1 - update binary-HT publication (#PPI >= 100)  
2 - additional criteria for selected evidence codes (6,7,19,59,61,75,96): if a publication with any of these codes detected PPI > threshold (25), it will be considered as co-complex instead of binary
